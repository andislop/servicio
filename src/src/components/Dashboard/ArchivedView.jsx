...
const ArchivedView = ({ usuariosInactivos, handleActive }) => {
  ...
  const filteredHeads = useMemo(() => {
    return usuariosInactivos.filter((head) => {
      const matchesSearch =
      ...
      const matchesStatus =
        filterStatus === "Todos" || head.status === filterStatus;
      return matchesSearch && matchesStatus;
    });
  }, [usuariosInactivos, searchTerm, filterStatus]);
  ...
  const firstItemDisplayed =
    filteredHeads.length > 0 ? indexOfFirstItem + 1 : 0;
