...
const Hero = () => {
    ...
    const fetchImages = async () => {
        ...
        try {
            ...
            if (!response.ok) {
                const errorBody = await response.json().catch(() => ({ message: 'Error de conexión.' }));
                throw new Error(errorBody.message || `Error ${response.status}: No se pudo cargar la galería.`);
            }
    ...
    if (error) {
        return <div className="text-center mt-32 p-10 text-red-600">❌ {error}</div>;
    }

    if (images.length === 0) {
        return <div className="text-center mt-32 p-10">No hay noticias o imágenes disponibles.</div>;
    }
                                ...
                                <div className="flex flex-col md:flex-row p-5 gap-8 h-full"> 
                                    ...
                                    <div className="w-full md:w-1/2 flex items-center order-last md:order-first h-full"> 
                                        <div className="text-left self-center">
                                            <h1 className="text-4xl font-bold text-texto-h1">{displayTitle}</h1> 
                                            <h3 className="text-2xl mt-2">
                                                {displayDescription.split('.')[0] + (displayDescription.includes('.') ? '.' : '')}
                                            </h3>
                                            <p className="mt-4">{displayDescription}</p>
                                        </div>
                                    </div>
                ...
                <div className="absolute bottom-4 left-1/2 -translate-x-1/2 flex space-x-2 ">
                    {images.map((_, index) => (
                        <button
                            ...
                            className={`w-3 h-3 rounded-full ${activeIndex === index ? 'bg-black' : 'bg-gray-400'}`}
                        ></button>
                    ))}
                </div>
            )}
